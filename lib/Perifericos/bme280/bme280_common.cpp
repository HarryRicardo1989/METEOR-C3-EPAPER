...
namespace CPPBME280
{
    ...
    float BME280::getDewPointTemperature(void)
    {
        BME280ResultData results{};
        ...
        const float a = 17.27;
        const float b = 237.7;
        float gamma = (a * results.temperature) / (b + results.temperature) + log(results.humididty / 100.0);
    ...
    esp_err_t BME280::writeByteData(const uint8_t reg, const uint8_t value)
    {

        return i2c->WriteRegister(_devAddress, reg, value);
    }

    int BME280::readByteData(const uint8_t reg)
    {

        return i2c->ReadRegister(_devAddress, reg);
    }

    int BME280::readWordData(const uint8_t reg)
    {
        uint8_t buff[2];
    ...
    esp_err_t BME280::readBlockData(const uint8_t reg, uint8_t *buf, const int length)
    {
        return i2c->ReadRegisterMultipleBytes(_devAddress, reg, buf, length);
    }

    void BME280::InitI2c(PROTOCOL::I2c *i_i2c, const uint8_t dev_addr)
    {
        i2c = i_i2c;
