...
#define EPD_DEEPSLEEP_MODE3 (0x11)
...
private:
    ...
    void epd_write_data_(const uint8_t *data, size_t length);
    void epd_write_data(uint8_t value);

    void epd_init_internalTempSensor(void);

    uint32_t _Pow(uint16_t m, uint16_t n);
};
